  ...
  while (XCheckWindowEvent (gstXInfo->xcontext->disp,
    ...
    switch (e.type) {
      ...
      case KeyRelease:
        ...
        g_mutex_lock (gstXInfo->x_lock);
        keysym = XKeycodeToKeysym (gstXInfo->xcontext->disp, e.xkey.keycode, 0);
        g_mutex_unlock (gstXInfo->x_lock);
        if (keysym != NoSymbol) {
          char *key_str = NULL;
    ...
    gint width, gint height, gint disp_width, gint disp_height)
{
  ...
  if (rw > 0) {
    XFillRectangle (gstXInfo->xcontext->disp, gstXInfo->xwindow->win,
        gstXInfo->xwindow->gc, lw + width, 0, rw, disp_height);
  }
  ...
  if (rh > 0) {
    XFillRectangle (gstXInfo->xcontext->disp, gstXInfo->xwindow->win,
        gstXInfo->xwindow->gc, 0, lh + height, disp_width, rh);
  }
...
mfw_gst_x11_xwindow_decorate (GstXInfo * gstXInfo, GstXWindow * window)
{
  ...
  hints_atom = XInternAtom (gstXInfo->xcontext->disp, "_MOTIF_WM_HINTS", True);
  if (hints_atom == None) {
    g_mutex_unlock (gstXInfo->x_lock);
    return FALSE;
  }
...
mfw_gst_x11_xwindow_new (GstXInfo * gstXInfo, gint width, gint height)
{
  ...
  xwindow->win = XCreateSimpleWindow (gstXInfo->xcontext->disp,
      ...
      0, 0, xwindow->width, xwindow->height, 0, 0, gstXInfo->xcontext->black);
  ...
  xwindow->gc = XCreateGC (gstXInfo->xcontext->disp, xwindow->win, 0, &values);
